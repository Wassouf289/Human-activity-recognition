...
def predict_live(output_file_path, window_size,model):
    ''' this function predicts the activity in live video from webcam,and writes the resulted
    video in output folder, the function uses deque object with the size of the number of 
    frames we want to average on over, because we don't want topredict for every single frame
    which leads to flickering, window_size=25: it means we will average the prediction for 
    every 25 frames'''
...
if __name__ == '__main__':
    ...
    predict_live(output_video_file_path, window_size,model)
